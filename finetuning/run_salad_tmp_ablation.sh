...
#   bash finetuning/run_salad_tmp_ablation.sh - 1
#   bash finetuning/run_salad_tmp_ablation.sh - 1,2
GPU_VISIBLE_LIST="${GPU_VISIBLE_LIST:-3}"
GPU_DEVICES="${2:-${GPU_VISIBLE_LIST}}"

PROJECT_ROOT="/media/data/zhangjingyi/ImAge"
PYTHON_BIN="${PYTHON_BIN:-python}"
TIMESTAMP="$(date +%m%d_%H%M%S)"
NOHUP_LOG="${PROJECT_ROOT}/nohup_run_salad_tmp_ablation_${TIMESTAMP}.log"

if [[ "${RUN_UNDER_NOHUP:-1}" == "1" && "${SALAD_ABLATION_NOHUP_LAUNCHED:-0}" != "1" ]]; then
  echo "[$(date '+%F %T')] launching under nohup..."
  echo "[$(date '+%F %T')] log: ${NOHUP_LOG}"
  SALAD_ABLATION_NOHUP_LAUNCHED=1 nohup bash "$0" "$@" > "${NOHUP_LOG}" 2>&1 &
  echo "[$(date '+%F %T')] background pid: $!"
  exit 0
fi

VARIANT_ORDER=(
  "all:salad_mixed_resume_tmp_all"
  "pitts:salad_mixed_resume_tmp_pitts"
  "msls:salad_mixed_resume_tmp_msls"
)
...
run_variant() {
  ...
  echo "[$(date '+%F %T')] starting Salad variant tmp_group=${tmp_group}, save_dir=${save_dir}, CUDA_VISIBLE_DEVICES=${GPU_DEVICES}"
  (
    cd "${PROJECT_ROOT}"
