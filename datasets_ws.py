...
for grouped_key, grouped_cfg in GROUPED_DATASETS.items():
    for query_set_name, query_dir in grouped_cfg["query_dirs"].items():
        GROUPED_QUERY_ALIASES[normalize_dataset_key(query_set_name)] = (
            grouped_key,
            query_set_name,
            query_dir,
        )
    for alias_name, alias_target in grouped_cfg.get("legacy_aliases", {}).items():
        GROUPED_QUERY_ALIASES[normalize_dataset_key(alias_name)] = (
            grouped_key,
            alias_target[0],
            alias_target[1],
        )
...
class BaseDataset(data.Dataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train"):
        ...
        normalized_name = normalize_dataset_key(dataset_name)
        grouped_cfg = GROUPED_DATASETS.get(normalized_name)
        grouped_alias = GROUPED_QUERY_ALIASES.get(normalized_name)

        if grouped_cfg is not None:
            self.dataset_name = grouped_cfg["dataset_name"]
            self.dataset_folder = join(datasets_folder, *grouped_cfg["path_parts"], "images", split)
            database_folder = join(self.dataset_folder, grouped_cfg["database_dir"])
            ...
            for query_set_name, query_dir in grouped_cfg["query_dirs"].items():
                query_folder = join(self.dataset_folder, query_dir)
                if not os.path.exists(query_folder):
                    raise FileNotFoundError(f"Folder {query_folder} does not exist")
                start_index = len(self.queries_paths)
                query_paths = sorted(glob(join(query_folder, "**", "*.jpg"), recursive=True))
                self.queries_paths.extend(query_paths)
                self.query_group_slices[query_set_name] = (start_index, len(self.queries_paths))
        elif grouped_alias is not None:
            grouped_key, query_set_name, query_dir = grouped_alias
            grouped_cfg = GROUPED_DATASETS[grouped_key]
            self.dataset_name = query_set_name
            self.dataset_folder = join(datasets_folder, *grouped_cfg["path_parts"], "images", split)
            database_folder = join(self.dataset_folder, grouped_cfg["database_dir"])
            queries_folder = join(self.dataset_folder, query_dir)
            if not os.path.exists(self.dataset_folder):
                raise FileNotFoundError(f"Folder {self.dataset_folder} does not exist")
            if not os.path.exists(database_folder):
                raise FileNotFoundError(f"Folder {database_folder} does not exist")
    ...
    def __repr__(self):
        if self.query_group_slices:
            ...
            return (
                ...
                f"#queries: {self.queries_num} ({query_breakdown}) >"
            )
